...
(ROC_Dengue / ROC_Dengue2 | ROC_Dengue4 / ROC_Dengue3 | ROC_Dengue5 / ROC_Dengue6 & theme(plot.tag = element_text(size = 12))
  ...
  plot_annotation(
    title = 'The performance of the complicated malaria signatures in dengue fever datasets',
    tag_levels = c('A', '1'),
...
##############################################
## TB 
png(filename = "./Figs/CompMalariaSigPerformance_TB.png", width = 1500, height = 1000, res = 100)
(ROC_TB1 / ROC_TB2 | ROC_TB3 / ROC_TB4 & theme(plot.tag = element_text(size = 12))
) +
  #plot_layout(widths = c(0.4, 1)) + 
  plot_annotation(
    title = 'The performance of the complicated malaria signatures in TB datasets',
    tag_levels = c('A', '1'),
    theme = theme(plot.title = element_text(size = 17, face = "bold"))
  )
dev.off()
...
##############################################
## Many infections
png(filename = "./Figs/CompMalariaSigPerformance_OtherInfections.png", width = 1500, height = 1000, res = 100)
(ROC_Adenovirus / ROC_ManyInfections1 | ROC_ManyInfections2 / ROC_ManyInfections3 | ROC_HIVandTB / ROC_WestNile & theme(plot.tag = element_text(size = 12))
  ...
  #plot_layout(widths = c(0.4, 1)) + 
  plot_annotation(
    title = 'The performance of the complicated malaria signatures in other viral and bacterial infections datasets',
    tag_levels = c('A', '1'),
    theme = theme(plot.title = element_text(size = 17, face = "bold"))
  )
dev.off()
