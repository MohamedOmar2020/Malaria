...
levels(pheno1$DiseaseStatus) <- c("control", "nonCerebral", "cerebral", "nonCerebral", "nonCerebral") 
table(pheno1$DiseaseStatus)

# Pheno2
pheno2$DiseaseStatus <- as.factor(pheno2$`diesease status:ch1`)
levels(pheno2$DiseaseStatus) <- c("control", "nonCerebral", "cerebral", "nonCerebral", "nonCerebral") 
table(pheno2$DiseaseStatus)

# Pheno3
pheno3$DiseaseStatus <- as.factor(pheno3$`diagnosis:ch1`)
levels(pheno3$DiseaseStatus) <- c("cerebral", "control", "nonCerebral") 
table(pheno3$DiseaseStatus)

# Pheno4
pheno4$DiseaseStatus <- as.factor(pheno4$`disease state:ch2`)
levels(pheno4$DiseaseStatus) <- c("control", "nonCerebral", "nonCerebral") 
table(pheno4$DiseaseStatus)

# Pheno5
pheno5$DiseaseStatus <- as.factor(pheno5$source_name_ch1)
levels(pheno5$DiseaseStatus) <- c("control", "nonCerebral") 
...
levels(pheno7$DiseaseStatus) <- c("nonCerebral", "control") 
...
levels(pheno8$DiseaseStatus) <- c("nonCerebral", "control") 
...
DiseaseStatus <- mapply(x=allpheno, FUN=function(x) {
  x <- x[,"DiseaseStatus"]
  out <- factor(x, levels=c("control", "nonCerebral", "cerebral"))
  out
})
